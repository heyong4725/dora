...
pub trait Component: Send + Sync {
    ...
    fn initialize<'a>(&'a mut self, _app_state: &'a AppState)
        -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async { Ok(()) })
    }

    /// Component cleanup
    fn cleanup<'a>(&'a mut self)
        -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async { Ok(()) })
    }
}
...
impl EventDispatcher {
    ...
    pub fn route_event(&self, event: &ComponentEvent, _registry: &ComponentRegistry) -> Option<ComponentId> {
