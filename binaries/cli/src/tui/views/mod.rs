...
pub trait View {
    ...
    fn update(&mut self, app_state: &mut AppState) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn on_mount(&mut self, app_state: &mut AppState) -> impl std::future::Future<Output = Result<()>> + Send {
        async { Ok(()) }
    }
